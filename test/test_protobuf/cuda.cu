...
namespace Test_cnn{
	...
	__global__ void im2col_bool_gpu_kernel(const int n, const float* data_im,
		const int height, const int width, const int kernel_h, const int kernel_w,
		const int pad_h, const int pad_w,
		const int stride_h, const int stride_w,
		const int dilation_h, const int dilation_w,
		...
		CUDA_KERNEL_LOOP(index, n) {
			...
			const int h_offset = h_col * stride_h - pad_h;
			const int w_offset = w_col * stride_w - pad_w;
			float* data_col_booled_ptr = data_col;
			data_col_booled_ptr += (c_col * height_col + h_col) * width_col + w_col;
			const float* data_im_ptr = data_im;
			data_im_ptr += (c_im * height + h_offset) * width + w_offset;
			for (int i = 0; i < kernel_h; ++i) {
				for (int j = 0; j < kernel_w; ++j) {
					...
					int w_im = w_offset + j * dilation_w;
					*data_col_booled_ptr =
						(h_im >= 0 && w_im >= 0 && h_im < height && w_im < width && (data_im_ptr[i * dilation_h * width + j * dilation_w] != 0)) ?
						1 : 0;
					data_col_booled_ptr += height_col * width_col;
				}
			}
		}
	}
	...
	void forward_conv_gpu(const float *_input, float *_im2col,
		...
		im2col_gpu(_input, _in_channels,
			_in_height, _in_width, _kernel_h, _kernel_w,
			pad, pad,
			stride, stride,
			1, 1,
			_im2col);
		...
		cublasSgemm_v2(handle, CUBLAS_OP_N, CUBLAS_OP_N,
			_in_with1, _out_high, _kenel_num,
			&alpha, _im2col, _in_with1,
			_kenel_data, _kenel_num, 
			&beta, _out,
			_in_with1);
	}
	...
	template void im2col_bool_gpu<float>(const float* data_im, const int channels,
		const int height, const int width, const int kernel_h, const int kernel_w,
		const int pad_h, const int pad_w, const int stride_h, const int stride_w,
		const int dilation_h, const int dilation_w, float* data_col);
	template void im2col_bool_gpu<double>(const double* data_im, const int channels,
		const int height, const int width, const int kernel_h, const int kernel_w,
		const int pad_h, const int pad_w, const int stride_h, const int stride_w,
		const int dilation_h, const int dilation_w, double* data_col);
